...
#include "fimc-is-cis-4h5yc-setA.h"
#include "fimc-is-cis-4h5yc-setB.h"

#include "fimc-is-helper-i2c.h"

#define SENSOR_NAME "S5K4H5YC"
/* #define DEBUG_4H5YC_PLL */
...
static u32 sensor_4h5yc_max_setfile_num;

static void sensor_4h5yc_cis_data_calculation(const struct sensor_pll_info *pll_info, cis_shared_data *cis_data)
{
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_4H5YC_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_4H5YC_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_4H5YC_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_4H5YC_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

static int sensor_4h5yc_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
/* CIS OPS */
int sensor_4h5yc_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	cis->cis_data->cur_height = SENSOR_4H5YC_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_4h5yc_cis_data_calculation(sensor_4h5yc_pllinfos[setfile_index], cis->cis_data);
...
int sensor_4h5yc_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
...
static int sensor_4h5yc_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_4h5yc_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_4h5yc_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
...
int sensor_4h5yc_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	if (mode > sensor_4h5yc_max_setfile_num) {
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_4h5yc_check_rev is fail");
			goto p_err;
		}
	}

	sensor_4h5yc_cis_data_calculation(sensor_4h5yc_pllinfos[mode], cis->cis_data);

	ret = sensor_cis_set_registers(subdev, sensor_4h5yc_setfiles[mode], sensor_4h5yc_setfile_sizes[mode]);
	if (ret < 0) {
...
int sensor_4h5yc_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_4h5yc_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_4H5YC_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_4H5YC_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_4H5YC_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_4H5YC_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_4H5YC_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_4H5YC_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_4h5yc_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
...
int sensor_4h5yc_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_4h5yc_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	hold = sensor_4h5yc_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
...
int sensor_4h5yc_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
...
int sensor_4h5yc_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_4h5yc_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_4h5yc_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_4h5yc_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	const struct sensor_pll_info *pll_info = NULL;
	...
	ret = sensor_4h5yc_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
	...
	if (cis_data->sens_config_index_cur <= sensor_4h5yc_max_setfile_num) {
		pll_info = sensor_4h5yc_pllinfos[cis_data->sens_config_index_cur];
	} else {
...
int sensor_4h5yc_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_4h5yc_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_4h5yc_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_4h5yc_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_4h5yc_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_4h5yc_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_4h5yc_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_4h5yc_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_4h5yc_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_4h5yc_cis_compensate_gain_for_extremely_br(struct v4l2_subdev *subdev, u32 expo, u32 *again, u32 *dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	if (pre_coarse_int <= 15 && pre_coarse_int != 0) {
		...
		dbg_sensor("[%s] exp(%d), again(%d), dgain(%d), coarse_int(%d), compensated_again(%d), integration_time : (%d)\n", __func__, expo, *again, *dgain, coarse_int, compensated_again, integration_time);
	}
...
static struct fimc_is_cis_ops cis_ops = {
	...
	.cis_stream_off = sensor_4h5yc_cis_stream_off,
	.cis_set_exposure_time = sensor_4h5yc_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_4h5yc_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_4h5yc_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_4h5yc_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_4h5yc_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_4h5yc_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_4h5yc_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_4h5yc_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_4h5yc_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_4h5yc_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_4h5yc_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_4h5yc_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_4h5yc_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_4h5yc_cis_get_min_digital_gain,
	...
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_S5K4H5YC);
	if (!sensor_peri) {
...
static int cis_4h5yc_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_4h5yc_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-4h5yc",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_4h5yc_match);

static const struct i2c_device_id cis_4h5yc_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver cis_4h5yc_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_4h5yc_match
	},
	.probe	= cis_4h5yc_probe,
	.remove	= cis_4h5yc_remove,
	.id_table = cis_4h5yc_idt
};
module_i2c_driver(cis_4h5yc_driver);
